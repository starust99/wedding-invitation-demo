...
5. External Apple Safari receives an inline iCalendar response; other clients
   receive a readable calendar-file attachment from the same route.
6. In-app browsers and ordinary non-Android browsers start a short non-blocking
   watch.
7. `pagehide`, blur, or a hidden document cancels the watch as a successful
   handoff signal.
8. If the page is still visible after the delay, one inline instruction appears
   below the existing buttons: external-browser recovery for WebViews, or
   downloaded-calendar-file recovery for ordinary browsers.
...
- Other clients return `200` with `text/calendar; charset=utf-8`; external
  Apple Safari uses `inline`, while remaining clients use `attachment`.
...
conditionally added only after a likely failed handoff; it never blocks or
replaces the links, and never asks the guest to interpret `.ics` terminology.
