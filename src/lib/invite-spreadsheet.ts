...
function insideInviteFormula(rowIndex: number, options: ReturnType<typeof resolveSpreadsheetOptions>) {
  ...
  const coupleInviteLine = `${excelText("Kính mời: ")}&${coupleInviteRecipientExpressionValue}&${excelText(" đến chung vui trong ngày chung đôi của ")}&${coupleInviteOwner}&${excelText(".")}`;
  const normalInviteLine = `IF(${invitedByCell}=${excelText(invitedByLabels.parents)},${hostSubjectExpressionValue}&${excelText(" trân trọng kính mời ")}&${recipientExpression}&${excelText(" đến chung vui trong ngày chung đôi của ")}&${coupleReferenceCell}&${excelText(` ${options.coupleDisplayName}.`)},IF(${isWarm},${hostSubjectExpressionValue}&${excelText(" mời ")}&${recipientExpression}&${excelText(" đến chung vui trong ngày chung đôi của ")}&${hostPronounCell}&${excelText(".")},${hostSubjectExpressionValue}&${excelText(" trân trọng kính mời ")}&${recipientExpression}&${excelText(" đến chung vui trong ngày chung đôi của ")}&${hostPronounCell}&${excelText(".")}))`;
