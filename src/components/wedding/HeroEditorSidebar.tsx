...
import { getHeroAssetDefinition, validateHeroAssetRatio } from "@/lib/wedding/hero-config";
import type { HeroAssetConfig, HeroAssetSlotId, HeroViewportMode } from "@/lib/wedding/hero-types";
...
const roleLabels: Record<HeroAssetConfig["role"], string> = {
  background: "Nền",
...
export function HeroEditorSidebar({
  assets,
  selectedAssetId,
  viewport,
  assetBucket,
  onSelect,
  onAssetChange,
}: HeroEditorSidebarProps) {
  const [uploadingId, setUploadingId] = useState<HeroAssetSlotId | null>(null);
  const [messages, setMessages] = useState<Partial<Record<HeroAssetSlotId, string>>>({});
  const inputRefs = useRef<Partial<Record<HeroAssetSlotId, HTMLInputElement | null>>>({});

  async function uploadAsset(asset: HeroAssetConfig, file: File | undefined) {
    if (!file) return;
    ...
    try {
      const prepared = await prepareImageFileForUpload(file, `hero-${asset.id}`);
      
      const size = await readImageSize(prepared.file).catch(() => null);
      const ratioWarning = size ? validateHeroAssetRatio(asset.id, size.width, size.height) : "";
      const formData = new FormData();
      formData.append("file", prepared.file);
      formData.append("section", `hero-${asset.id}`);
  ...
  return (
    <aside className="grid max-h-[calc(100vh-9rem)] content-start gap-3 overflow-y-auto rounded-[2rem] border border-white/60 bg-white/40 p-4 shadow-[0_24px_80px_rgba(92,82,71,0.10)] backdrop-blur-xl">
      <div>
        <p className="font-sans text-[11px] font-bold uppercase tracking-[0.24em] text-text-primary/55">13 vị trí ảnh</p>
        <h2 className="mt-1 font-serif text-3xl italic text-text-primary">Ảnh phần đầu thiệp</h2>
      </div>

      {assets.map((asset) => {
        const placement = viewport === "mobile" ? asset.mobile : asset.desktop;
        const selected = asset.id === selectedAssetId;
        const definition = getHeroAssetDefinition(asset.id);
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <span
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  const nextPlacement = { ...placement, visible: !placement.visible };
                  onAssetChange({
                    ...asset,
                    [viewport]: nextPlacement,
                  });
                }}
                onKeyDown={(event) => {
                  if (event.key !== "Enter" && event.key !== " ") return;
