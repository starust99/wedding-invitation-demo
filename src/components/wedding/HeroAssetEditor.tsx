...
import { HeroEditorControls } from "@/components/wedding/HeroEditorControls";
import { HeroEditorSidebar } from "@/components/wedding/HeroEditorSidebar";
import { WeddingHero } from "@/components/wedding/WeddingHero";
import {
  createDefaultWeddingHeroConfig,
  normalizeWeddingHeroEditorConfig,
  touchWeddingHeroConfig,
  weddingHeroEditorExportFileName,
  weddingHeroEditorStorageKey,
} from "@/lib/wedding/hero-config";
import {
  smartArrangeWeddingHeroConfig,
  type HeroAutoArrangeMode,
  type HeroAutoLayoutReportItem,
} from "@/lib/wedding/hero-auto-layout";
import type {
  HeroAssetConfig,
  HeroAssetSlotId,
  HeroViewportMode,
...
type HeroAssetEditorProps = {
  config: WeddingHeroEditorConfig;
  assetBucket?: string;
  onChange: (config: WeddingHeroEditorConfig) => void;
...
function downloadJson(config: WeddingHeroEditorConfig) {
  const blob = new Blob([JSON.stringify(config, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = weddingHeroEditorExportFileName;
  anchor.click();
  URL.revokeObjectURL(url);
}

function getStageTarget(mode: HeroViewportMode) {
  return mode === "mobile"
    ? { width: 390, height: 844 }
    : { width: 1440, height: 810 };
}

function getAssetById(config: WeddingHeroEditorConfig, id: HeroAssetSlotId) {
  return config.assets.find((asset) => asset.id === id) || config.assets[0];
}

function formatSmartScope(scope: HeroAutoArrangeMode) {
  if (scope === "both") return "màn hình lớn + di động";
  if (scope === "desktop") return "màn hình lớn";
  return "di động";
}

function formatSmartArrangeNotice(report: HeroAutoLayoutReportItem[], scope: HeroAutoArrangeMode) {
  const patched = report.filter((item) => item.changed.length > 0);
  const warnings = report.filter((item) => item.changed.length === 0);

  if (patched.length === 0 && warnings.length === 0) {
    return `Tối ưu phần đầu thiệp (${formatSmartScope(scope)}): bố cục đang ổn, không cần sửa.`;
  }

  const details = report
    .slice(0, 4)
    .map((item) => `${item.viewport}/${item.id}: ${item.reason}`)
    .join(" · ");
  const extra = report.length > 4 ? ` · +${report.length - 4} mục nữa` : "";
  const warningText = warnings.length ? ` Có ${warnings.length} cảnh báo chưa tự sửa.` : "";

  return `Tối ưu phần đầu thiệp (${formatSmartScope(scope)}) đã sửa ${patched.length} vị trí.${warningText} ${details}${extra}`;
}

type HeroEditorCanvasPanelProps = {
  config: WeddingHeroEditorConfig;
  viewport: HeroViewportMode;
  selectedAssetId: HeroAssetSlotId;
  showSafeGuides: boolean;
  fullscreen?: boolean;
  onViewportChange: (viewport: HeroViewportMode) => void;
...
function HeroEditorCanvasPanel({
  config,
  viewport,
  selectedAssetId,
  showSafeGuides,
  ...
  return (
    <section
      className={[
        ...
        fullscreen ? "p-3" : "p-4",
        fullscreen ? "min-h-0 grid-rows-[auto_minmax(0,1fr)_auto]" : "",
      ].join(" ")}
    >
      ...
      <p className="text-xs leading-5 text-text-primary/50">
        Bấm ảnh để chọn. Kéo ảnh để đổi x/y, kéo tay nắm để đổi rộng. Phím mũi tên dịch 0.5%, Shift+phím mũi tên dịch 2%. Delete ẩn ảnh tùy chọn.
      </p>
    </section>
  );
}

export function HeroAssetEditor({ config, assetBucket, onChange, onSaveDraft }: HeroAssetEditorProps) {
  const normalizedConfig = useMemo(() => normalizeWeddingHeroEditorConfig(config), [config]);
  const [selectedAssetId, setSelectedAssetId] = useState<HeroAssetSlotId>("heroBgWash");
  ...
  function commit(nextConfig: WeddingHeroEditorConfig, message = "") {
    onChange(touchWeddingHeroConfig(normalizeWeddingHeroEditorConfig(nextConfig)));
    if (message) setNotice(message);
  }

  function updateAsset(nextAsset: HeroAssetConfig) {
    commit({
      ...normalizedConfig,
  ...
  function updatePlacement(id: HeroAssetSlotId, placement: ResponsiveAssetPlacement) {
  ...
  function nudgeSelected(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if (event.key === "Delete" || event.key === "Backspace") {
      if (asset.required) {
        setNotice("Vị trí bắt buộc không xóa bằng Delete. Có thể tắt hiển thị hoặc thay ảnh.");
        return;
      }
      updateAsset({
        ...asset,
  ...
  async function saveConfig() {
    window.localStorage.setItem(weddingHeroEditorStorageKey, JSON.stringify(normalizedConfig));
    await onSaveDraft?.();
    setNotice("Đã lưu cấu hình phần đầu thiệp vào bộ nhớ cục bộ và bản nháp.");
  }

  function loadConfig() {
    const raw = window.localStorage.getItem(weddingHeroEditorStorageKey);
    if (!raw) {
      setNotice("Chưa có cấu hình lưu cục bộ.");
      return;
    }
    commit(JSON.parse(raw) as WeddingHeroEditorConfig, "Đã tải cấu hình phần đầu thiệp từ bộ nhớ cục bộ.");
  }

  function resetAll() {
    commit(createDefaultWeddingHeroConfig(), "Đã đặt lại toàn bộ bố cục phần đầu thiệp.");
    setSelectedAssetId("heroBgWash");
  }

  async function smartArrange(scope: HeroAutoArrangeMode) {
    setArranging(true);

    try {
      const result = await smartArrangeWeddingHeroConfig(normalizedConfig, scope);
      commit(result.config, formatSmartArrangeNotice(result.report, scope));
    } catch {
      setNotice("Tối ưu tự động bị lỗi khi đọc ảnh hoặc tính bố cục. Thử tải lại trình sửa rồi chạy lại.");
    } finally {
      setArranging(false);
    }
  }
  ...
  async function importJson(file: File | undefined) {
    if (!file) return;
    try {
      const imported = JSON.parse(await file.text()) as WeddingHeroEditorConfig;
      commit(imported, "Đã nhập tệp JSON cấu hình phần đầu thiệp.");
    } catch {
      ...
      <div className="fixed inset-0 z-[90] flex flex-col bg-pastel-cream/95 text-text-primary backdrop-blur-xl">
        <header className="flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-text-primary/10 bg-white/35 px-4 py-3 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => smartArrange(viewport)}
              disabled={arranging}
              className="inline-flex min-h-10 items-center gap-2 rounded-full bg-pastel-serenity/70 px-4 text-xs font-bold uppercase tracking-[0.16em] text-text-primary/75 disabled:cursor-not-allowed disabled:opacity-55"
            >
  ...
  return (
    <div className="grid gap-4 rounded-[2.25rem] border border-white/60 bg-pastel-cream/70 p-4 text-text-primary shadow-[0_30px_100px_rgba(92,82,71,0.10)] backdrop-blur-xl xl:col-span-2">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="font-sans text-xs font-bold uppercase tracking-[0.28em] text-text-primary/55">Trình sửa ảnh phần đầu thiệp</p>
          <h2 className="mt-1 font-serif text-4xl italic text-text-primary">Bố cục phần đầu thiệp</h2>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => smartArrange("both")} disabled={arranging} className="inline-flex min-h-10 items-center gap-2 rounded-full bg-pastel-serenity/70 px-4 text-xs font-bold uppercase tracking-[0.18em] text-text-primary disabled:cursor-not-allowed disabled:opacity-55">
            {arranging ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
            Tối ưu phần đầu thiệp
          </button>
          <button type="button" onClick={() => smartArrange("desktop")} disabled={arranging} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/70 bg-white/40 px-4 text-xs font-bold uppercase tracking-[0.18em] text-text-primary/70 disabled:cursor-not-allowed disabled:opacity-55">
            <Monitor className="h-4 w-4" />
            Tối ưu màn lớn
          </button>
          <button type="button" onClick={() => smartArrange("mobile")} disabled={arranging} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/70 bg-white/40 px-4 text-xs font-bold uppercase tracking-[0.18em] text-text-primary/70 disabled:cursor-not-allowed disabled:opacity-55">
