...
assert(salutationOptions.includes("Ba"));
...
sheet.getCell("B11").value = "Ba";
sheet.getCell("F11").value = "[Nhà Gái] Họ nội";
...
assert.equal(parsed.invitees.length, 7);
...
assert.equal(parsed.invitees[6].displayLabel, "Ba");
assert.equal(parsed.invitees[6].salutationCluster, "Ba");
assert.equal(parsed.invitees[6].guestName, "Ba");
assert.equal(parsed.invitees[6].honorific, "ba");
assert.equal(parsed.invitees[6].householdMode, "single");
assert.equal(parsed.invitees[6].expectedGuestCount, 1);
assert.equal(parsed.invitees[6].terracottaLodgingEligible, true);
